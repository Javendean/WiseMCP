...
from . import database, models, tools, exceptions
...
async def get_tools() -> List[models.Tool]:
    ...
    """
    return tools.AVAILABLE_TOOLS_SCHEMA
...
async def execute_tool(
    ...
    tool_func = tools.TOOL_FUNCTIONS.get(request.tool_name)
    if not tool_func:
    ...
    else: # search_internal_knowledge_base doesn't need db_session or conversation_id
    ...
    try:
        ...
        return models.ToolExecutionResponse(
            ...
            result=str(result) # Ensure result is always a string
        )
    except exceptions.ToolExecutionError as e:
        # Re-raise custom exceptions to be caught by the global handler
        raise e
